...
void kinc_g4_render_target_get_pixels(kinc_g4_render_target_t *renderTarget, uint8_t *data) {
	...
	ID3D11Texture2D *sourceTexture = renderTarget->impl.textureRender;
	if (renderTarget->impl.textureSample != NULL && renderTarget->impl.textureSample != renderTarget->impl.textureRender) {
		dx_ctx.context->lpVtbl->ResolveSubresource(dx_ctx.context, (ID3D11Resource *)renderTarget->impl.textureSample, 0,
		                                           (ID3D11Resource *)renderTarget->impl.textureRender, 0,
		                                           convertRenderTargetFormat((kinc_g4_render_target_format_t)renderTarget->impl.format));
		sourceTexture = renderTarget->impl.textureSample;
	}
	...
	dx_ctx.context->lpVtbl->CopySubresourceRegion(dx_ctx.context, (ID3D11Resource *)renderTarget->impl.textureStaging, 0, 0, 0, 0,
	                                              (ID3D11Resource *)sourceTexture, 0, &sourceRegion);
	...
	int bytesPerPixel = formatRenderTargetByteSize((kinc_g4_render_target_format_t)renderTarget->impl.format);
	int packedRowSize = renderTarget->texWidth * bytesPerPixel;

	if (mappedResource.RowPitch == packedRowSize) {
		memcpy(data, mappedResource.pData, packedRowSize * renderTarget->texHeight);
	}
	else {
		uint8_t *source = (uint8_t *)mappedResource.pData;
		for (int y = 0; y < renderTarget->texHeight; ++y) {
			memcpy(data + (y * packedRowSize), source + (y * mappedResource.RowPitch), packedRowSize);
		}
	}
