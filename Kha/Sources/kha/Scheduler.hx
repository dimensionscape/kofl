...
class Scheduler {
	...
	#if sys
	static function __log(message:String):Void {
	...
	public static function addBreakableTimeTaskToGroup(groupId: Int, task: Void->Bool, start: Float, period: Float = 0, duration: Float = 0): Int {
		ensureInitialized();
		#if sys
		__log('Scheduler.addBreakableTimeTaskToGroup before newTask group=' + groupId + ' start=' + start + ' period=' + period + ' timeTasksNull=' + (timeTasks == null));
		#end
		var t = new TimeTask();
		t.active = true;
		t.task = task;
		...
		t.next = t.start;
		#if sys
		__log('Scheduler.addBreakableTimeTaskToGroup before insert current=' + current + ' next=' + t.next + ' currentTimeTaskId=' + currentTimeTaskId);
		#end
		insertSorted(timeTasks, t);
		#if sys
		__log('Scheduler.addBreakableTimeTaskToGroup after insert id=' + t.id + ' length=' + timeTasks.length);
		#end
		return t.id;
	}
