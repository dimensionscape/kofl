			...
			var constructor = macro
				{
					...
					var bitmapData = openfl.Assets.getBitmapData(assetPath);
					if (bitmapData != null)
					{
						__copyFrom(bitmapData);
					}
					else
					{
				...
				if (meta.params.length > 0)
				{
					switch (meta.params[0].expr)
					{
						case EConst(CString(filePath)):
							...
							var assetPathValue = {pos: position, expr: EConst(CString(filePath))};
							fields.push({
								kind: FVar(macro :String, assetPathValue),
								name: "assetPath",
								access: [APrivate, AStatic],
								pos: position
							});
